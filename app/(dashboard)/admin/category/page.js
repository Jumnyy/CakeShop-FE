...
export default function CategoryPage() {
  ...
  return (
    <div className="p-8 bg-[#f8fafc] min-h-screen font-sans">
      {/* HEADER */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 bg-gradient-to-r from-emerald-600 to-teal-600 p-6 rounded-2xl shadow-lg text-white">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">
            Quản Lý Danh Mục
          </h1>
          <p className="text-teal-100 text-sm mt-1">
            Sắp xếp và tổ chức danh mục sản phẩm của bạn.
          </p>
        </div>
        <Link
          href="/admin/category/add"
          className="flex items-center bg-white text-emerald-600 px-5 py-2.5 rounded-xl hover:bg-emerald-50 transition shadow-md"
        >
      ...
      {/* FILTER BAR */}
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-wrap gap-4 mb-6 items-center">
        <input
          type="text"
          placeholder="Tìm kiếm danh mục..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="flex-1 min-w-[300px] pl-4 pr-4 py-2 rounded-xl border border-gray-300 bg-slate-50 focus:ring-2 focus:ring-emerald-500 outline-none transition"
        />
        <select
          value={limit}
          onChange={(e) => setLimit(Number(e.target.value))}
          className="bg-slate-50 border border-gray-300 text-slate-600 py-2 px-4 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none"
        >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="min-w-full text-left">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-20">
                ID
              </th>
